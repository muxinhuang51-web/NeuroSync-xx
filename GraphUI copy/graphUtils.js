class GraphManager {
    ...
    async GraphExtension(nodeId) {
        try {
            ...
            const response = await fetch('http://localhost:5001/api/graph_extension', {
                method: 'POST',
                headers: {
    ...
    renderGraph(svg, graph, isEditable, isInitialRender = false, previewScale = 1, commonNodeIds = null, disableExtension = false, disableEditing = false, modifiedNodeIds = null) {
        ...
        function ticked() {
            // Update edge paths with proper arrow positioning for rectangular nodes
            edgePaths.attr('d', d => {
                ...
                // Calculate the point where the edge intersects the rectangle
                // Half width and half height of the rectangle
                const halfWidth = nodeWidth / 2;
                const halfHeight = nodeHeight / 2;
                
                // Calculate edge points relative to node centers
                ...
                if (Math.abs(dx) * halfHeight > Math.abs(dy) * halfWidth) {
                    ...
                    const sign = dx > 0 ? 1 : -1;
                    startX = sourceX + sign * halfWidth;
                    startY = sourceY + (dy / dx) * sign * halfWidth;
                } else {
                    // Intersects with top/bottom side
                    const sign = dy > 0 ? 1 : -1;
                    startX = sourceX + (dx / dy) * sign * halfHeight;
                    startY = sourceY + sign * halfHeight;
                }
                
                // For target rectangle
                if (Math.abs(dx) * halfHeight > Math.abs(dy) * halfWidth) {
                    // Intersects with left/right side
                    const sign = dx > 0 ? -1 : 1;
                    endX = targetX + sign * halfWidth;
                    endY = targetY + (dy / dx) * sign * halfWidth;
                } else {
                    // Intersects with top/bottom side
                    const sign = dy > 0 ? -1 : 1;
                    endX = targetX + (dx / dy) * sign * halfHeight;
                    endY = targetY + sign * halfHeight;
                }
