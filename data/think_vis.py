...
# Define colors
baseline_color = '#FAA254'  # Blue
lancet_color =   '#4AA5D9'  # Salmon/red
modify_color = '#D654FA'    # Purple
...
for outlier in b_tot_outliers:
    ax1.plot(x[0], outlier, 'ro', ms=10, markeredgecolor='black', zorder=5)
for outlier in l_tot_outliers:
    ax1.plot(x[1], outlier, 'ro', ms=10, markeredgecolor='black', zorder=5)
...
ax1.text(0.5, max(baseline_total_time, lancet_total_time) , 
...
ax1.set_xticks(x)
ax1.set_xticklabels(['Baseline', 'NeuroSync'])
...
for outlier in b_call_outliers:
    ax2.plot(x[0], outlier, 'ro', ms=5, markeredgecolor='black', zorder=5)
for outlier in l_call_outliers:
    ax2.plot(x[1], outlier, 'ro', ms=5, markeredgecolor='black', zorder=5)
...
ax2.text(0.5, max(baseline_llm_calls, effective_lancet_calls) , 
...
ax2.set_xticks(x)
ax2.set_xticklabels(['Baseline', 'NeuroSync'])
...
time_legend_elements = [
    ...
    Patch(facecolor=lancet_color, edgecolor='black', label='NeuroSync Total Time'),
    ...
    Line2D([0], [0], marker='o', color='none', markerfacecolor='r', 
...
calls_legend_elements = [
    ...
    Patch(facecolor=lancet_color, edgecolor='black', label='Reason LLM Call'),
    Patch(facecolor=modify_color, edgecolor='black', label='Modify LLM Call'),
    Line2D([0], [0], marker='o', color='none', markerfacecolor='r', 
           markeredgecolor='black', markersize=10, label='Outlier')
]

# Comment out or remove these lines to hide all legends
