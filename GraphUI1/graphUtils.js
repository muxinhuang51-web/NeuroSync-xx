class GraphManager {
    ...
    async GraphExtension(nodeId) {
        try {
            ...
            console.log('GraphExtension Result:', result);
            
            if (result.status === "success") {
                return result.subgraph;
            } else {
                console.error('Failed to extend graph:', result.message);
                return null;
            }
        } catch (error) {
            console.error('Failed to test GraphExtension:', error);
            return null;
        }
    }   

    // Update this method to properly handle the response
    async handleNodeExtension(node) {
        try {
            // Extract node ID
            const nodeId = node.id;
            this.logUserAction(`Extending/collapsing node ${nodeId}`);
            
            // Check if this node is already extended (has a "-" sign)
            if (node.isExtended) {
                // If extended, collapse it
                this.logUserAction(`Collapsing node ${nodeId}`);
                
                // Remove highlighting from the top graph
                this.oldGraphSvg.selectAll('.node rect')
                    .style('stroke', d => {
                        if (this.commonNodeIds && this.commonNodeIds.has(d.id)) {
                            return '#2196f3'; // Return to blue for common nodes
                        }
                        return '#ccc'; // Return to default stroke for other nodes
                    })
                    .style('stroke-width', '1px')
                    .style('fill', d => {
                        if (this.commonNodeIds && this.commonNodeIds.has(d.id)) {
                            return '#e3f2fd'; // Return to light blue for common nodes
                        }
                        return 'white'; // Return to default fill
                    });
                
                // Update the node state
                node.isExtended = false;
                
                // Update the extension indicator to "+" sign
                const extensionGroup = d3.select(d3.select(node).node().parentNode);
                extensionGroup.select('.extension-symbol').text('+');
                extensionGroup.select('.extension-indicator').style('fill', '#4caf50'); // Green color for "+"
                
                return;
            }
            
            // If not extended, extend it by calling the API
            ...
            if (result.status === "success" && result.subgraph) {
                console.log("Received subgraph:", result.subgraph);
                
                // Extract node IDs from the subgraph to highlight in the top graph
                const highlightNodeIds = new Set(result.subgraph.nodes.map(n => n.id));
                
                // Highlight the corresponding nodes in the top graph
                this.oldGraphSvg.selectAll('.node rect')
                    .style('stroke', d => highlightNodeIds.has(d.id) ? '#ff9800' : 
                        (this.commonNodeIds && this.commonNodeIds.has(d.id) ? '#2196f3' : '#ccc'))
                    .style('stroke-width', d => highlightNodeIds.has(d.id) ? '2.5px' : '1px')
                    .style('fill', d => highlightNodeIds.has(d.id) ? '#ffeedb' : 
                        (this.commonNodeIds && this.commonNodeIds.has(d.id) ? '#e3f2fd' : 'white'));
                ...
                const extensionGroup = d3.select(d3.select(node).node().parentNode);
                extensionGroup.select('.extension-symbol').text('-');
                extensionGroup.select('.extension-indicator').style('fill', '#f44336'); // Red color for "-"
            } else {
                console.error('Failed to get node extension:', result.message || 'Unknown error');
                alert("Unable to extend node: " + (result.message || "Unknown error"));
            }
        } catch (error) {
            console.error("Error during node extension:", error);
            alert("Node extension error: " + error.message);
        }
    }
    ...
    renderIntentTree(intentTree, changedNodes = []) {
        const container = document.getElementById('intentTreeContent');
        if (!container) return;
        
        container.innerHTML = '';
        
        // Convert changedNodes to Set for faster lookups
        const changedNodesSet = new Set(changedNodes);
        
        // Helper function to recursively render the tree
        const renderNode = (key, value, level = 0) => {
            const nodeDiv = document.createElement('div');
            nodeDiv.className = 'intent-node';
            // Check if this node should be highlighted (is in changedNodes)
            if (changedNodesSet.has(key)) {
                nodeDiv.classList.add('changed');
            }
            ...
            if (typeof value === 'object' && Object.keys(value).length > 0) {
                ...
                childrenDiv.className = 'intent-children';
                
                Object.entries(value).forEach(([childKey, childValue]) => {
                    childrenDiv.appendChild(renderNode(childKey, childValue, level + 1));
                });
        ...
        Object.entries(intentTree).forEach(([key, value]) => {
            container.appendChild(renderNode(key, value));
        });
    }
    ...
    renderGraph(svg, graph, isEditable, isInitialRender = false, previewScale = 1, commonNodeIds = null, disableExtension = false, disableEditing = false, modifiedNodeIds = null) {
        ...
        svg.append("defs").append("marker")
            ...
            .attr("refX", 15)  // Adjust to position arrow properly
            .attr("refY", 0)
            .attr("markerWidth", 8)  // Increase size for better visibility
            .attr("markerHeight", 8) // Increase size for better visibility
            .attr("orient", "auto")
            .append("path")
        ...
        const nodeWidth = 150 * previewScale;  // CHANGE: Same width regardless of preview
        const nodeHeight = 60 * previewScale;  // CHANGE: Same height regardless of preview
        ...
        // Ensure edges always have directional arrows visible by default
        if (graph && graph.edges) {
            graph.edges.forEach(edge => {
                edge.showArrow = true; // Explicitly mark arrows to be shown
            });
        }
    
        // FIX: Properly create edge paths with immediate arrow visibility
        const edgePaths = svg.selectAll('.edge')
            .data(graph.edges)
            .enter()
            .append('path')
            .attr('class', 'edge')
            .attr('marker-end', `url(#${markerId})`)  // Ensure this is set
            .style('stroke', '#999')
            .style('stroke-width', '2px')  // Increase from 1.5px to 2px
            .style('fill', 'none');

        // Then, modify the ticked function to ensure marker visibility
        function ticked() {
            // Update node positions
            nodes.attr("transform", d => `translate(${d.x},${d.y})`);
            
            // Update edge paths, ensuring the marker remains visible
            edgePaths.attr("d", function(d) {
                ...
                // If nodes are too close, draw a minimal line
                if (length === 0) return `M${sourceX},${sourceY}L${targetX},${targetY}`;
                
                // Shorten the line to not overlap with the target node
                // This ensures the arrow is visible at the edge of the node
                const offsetX = dx * (1 - (nodeWidth/2) / length);
                const offsetY = dy * (1 - (nodeHeight/2) / length);
                
                return `M${sourceX},${sourceY}L${sourceX + offsetX},${sourceY + offsetY}`;
            });
            
            // Update edge labels
            edgeLabels.attr("transform", function(d) {
                if (!d.source.x || !d.target.x) return "translate(0,0)";
        ...
        if (isEditable && !disableExtension) {
            ...
            // Add indicator for extended nodes
            nodes.filter(d => d.isExtended)
                .append('circle')
                .attr('class', 'extension-indicator')
                .attr('r', 8)
        ...
        const nodeLabels = nodes.append('text')
            ...
            .attr('text-anchor', 'middle')
            .attr('dominant-baseline', 'middle') // Changed from 'upper-left' to 'middle'
            .attr('dy', '-0.5em') // Changed from '-0.2em' to '-0.5em' to move text higher
            .style('font-size', fontSize + 'px')
            .style('fill', '#333');
        ...
        // Run simulation with proper initialization
        if (isInitialRender) {
            // Run the simulation with high alpha for better initialization
            simulation.alpha(1.0);
            
            // Run many ticks immediately to ensure proper positioning
            for (let i = 0; i < 100; i++) {
                simulation.tick();
            }
            
            // Call ticked explicitly to update visual elements
            ticked();
            
            // Then restart for smoother animations afterward
            simulation.alpha(0.3).restart();
        } else {
            // For updates, run some ticks immediately
            for (let i = 0; i < 30; i++) {
                simulation.tick();
            }
            // Update visuals
        ...
        // Add to renderGraph method, after commonNodeIds is passed in
        if (commonNodeIds) {
            this.commonNodeIds = commonNodeIds;
        }
    }
