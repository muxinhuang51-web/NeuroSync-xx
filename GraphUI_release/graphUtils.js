class GraphManager {
    ...
    async handleNodeExtension(node) {
        try {
            this.logUserAction(`Extending node: ${node.id}`);
            ...
            const result = await response.json();
            console.log('Graph extension result:', result);
            
            if (result.status === "success" && result.subgraph) {
                // Format the returned subgraph for display
                const formattedSubgraph = {
                    nodes: (result.subgraph.nodes || []).map(node => ({
                        id: node.id,
                        label: node.description || node.label || node.id
                    })),
                    edges: (result.subgraph.links || []).map(link => ({
                ...
                // Show the extension preview with the subgraph
                this.showExtensionPreview(null, formattedSubgraph, node);
            } else {
                console.error('Failed to extend node:', result.message || 'Unknown error');
                alert(`Failed to extend node: ${result.message || 'Unknown error'}`);
            }
        } catch (error) {
            console.error('Error extending node:', error);
            alert(`Error extending node: ${error.message}`);
        }
    }
    ...
    //         // Check if this node is in the changed nodes list
    ...
    //                 childNodeDiv.className = 'intent-node';
                    
    //                 // Check if this child node is in the changed nodes list
    //                 if (changedNodes.includes(childKey)) {
    //                     childNodeDiv.classList.add('changed');
    //                 }
                    
    //                 childNodeDiv.textContent = childKey;
    //                 childrenDiv.appendChild(childNodeDiv);
                    
    //                 // Recursively render grandchildren if any
    //                 if (childValue && typeof childValue === 'object' && Object.keys(childValue).length > 0) {
    //                     const grandchildrenDiv = document.createElement('div');
    //                     grandchildrenDiv.className = 'intent-children';
    //                     childrenDiv.appendChild(grandchildrenDiv);
                        
    //                     Object.entries(childValue).forEach(([grandchildKey, grandchildValue]) => {
    //                         renderNode(grandchildKey, grandchildValue, level + 2);
    //                     });
    //                 }
    //             });
    //         }
