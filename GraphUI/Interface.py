...
class Storage:
        ...
        try:
            ...
            while not success and attempt < max_attempts:
                ...
                try:
                    ...
                    intent_tree_json = extract_json_section(response_text, "### Intent Tree")
                    task_graph_json = extract_json_section(response_text, "### Task Graph")
                    mapping_json = extract_json_section(response_text, "### Mapping")

                    # Add error handling for JSON parsing
                    try:
                        self.intent_tree = json.loads(intent_tree_json) if intent_tree_json else {}
                        self.graph = json.loads(task_graph_json) if task_graph_json else {"nodes": [], "links": []}
                        self.mapping = json.loads(mapping_json) if mapping_json else {}
                    except Exception as e:
    ...
    def generate_graph(self, prompt):
        if self.test:
            ...
            self.graph = {
                            "nodes": [
                                {"id": "Node1", "description": "Fetch target webpage content via HTTP"},
                                {"id": "Node2", "description": "Parse HTML with BeautifulSoup"},
                                {"id": "Node3", "description": "Extract main text from web articles"},
                                {"id": "Node4", "description": "Save to Markdown"},
                                {"id": "Node5", "description": "Find all images in web articles"}
                            ],
                            "links": [
                                {"source": "Node1", "target": "Node2", "type": "parse"},
                                {"source": "Node2", "target": "Node3", "type": "extractText"},
            ...
            # self.sim_graph= {
            #                 "nodes": [
            #                     {"id": "Node1", "description": "Merged nodes from intent: Extract main text from web articles"},
            #                     {"id": "Node2", "description": "Merged nodes from intent: Find all images in web articles"},
            #                     # {"id": "Node6", "description": "Merged node"},
            #                 ],
            #                 "links": [
            #                     {"source": "Node1", "target": "Node2", "type": "parse"},
            #                     {"source": "Node2", "target": "Node6", "type": "extractText"},
            ...
            self.intent_tree = {
                            "Fetch target webpage content via HTTP": {
                                "Parse HTML with BeautifulSoup": {
                                    "Extract main text from web articles": {},
                                    "Find all images in web articles": {}
                                }
                            }
                        }
            self.mapping = {
...
def call_LLM(chat_history, new_graph):
    ...
    try:
        ...
        if storage_instance.baseline:
            ...
            user_prompt += f"Task graph:\n{json.dumps(new_graph, indent=2, ensure_ascii=False)}\n\n"
            user_prompt += "You need to generate a complete code implementation based on the task execution logic and subtask completion and user requirements described in the task graph above. Please ensure that the code can actually run and meet user requirements. At the same time, mark the code and give the corresponding relationship between the nodes in the graph."
...
def graph_extension(node_id):
    ...
    for node in storage_instance.sim_graph.get("nodes", []):
        if node["id"] == node_id:
            ...
            if "Merged intent: " in description:
                intent_name = description.split("Merged intent: ")[1].strip()
            break
...
def modify(user_input, new_graph):
    ...
    new_intent_tree= {'Fetch target webpage content via HTTP': {'Parse HTML with BeautifulSoup': {'Extract main text and titles from web articles': {},
                                                                                'Find all images in web articles': {},
                                                                                'Save images and location in articles': {}}}}

    new_graph={'links': [{'source': 'Node1', 'target': 'Node2', 'type': 'parse'},
            {'source': 'Node2', 'target': 'Node3', 'type': 'extractText'},
            {'source': 'Node3', 'target': 'Node4', 'type': 'add'},
            {'source': 'Node2', 'target': 'Node5', 'type': 'findImages'},
            {'source': 'Node2', 'target': 'Node6', 'type': 'relation'},
            {'source': 'Node6', 'target': 'Node4', 'type': 'relation'},
            {'source': 'Node5', 'target': 'Node7', 'type': 'saveImagesLocation'},
            {'source': 'Node7', 'target': 'Node8', 'type': 'saveImages'},
            {'source': 'Node7', 'target': 'Node4', 'type': 'add'}],
    'nodes': [{'description': 'Fetch target webpage content via HTTP',
                'id': 'Node1'},
            {'description': 'Parse HTML with BeautifulSoup', 'id': 'Node2'},
            {'description': 'Extract main text from web articles',
                'id': 'Node3'},
            {'description': 'Save text with titles+image location to .md', 'id': 'Node4'},
            {'description': 'Find all images in web articles', 'id': 'Node5'},
            {'description': 'get different level title', 'id': 'Node6'},
            {'description': 'Save images location in articles', 'id': 'Node7'},
            {'description': 'Save images in a floder', 'id': 'Node8'}]}

    new_mapping =    {'Extract main text and titles from web articles': {'nodes': ['Node1', 'Node2', 'Node3', 'Node4', 'Node6']},
    'Fetch target webpage content via HTTP': {'nodes': ['Node1', 'Node2']},
    'Find all images in web articles': {'nodes': ['Node1',
                                                'Node2',
                                                'Node4',
                                                'Node5',
                                                'Node7',
                                                'Node8']},
    'Parse HTML with BeautifulSoup': {'nodes': ['Node2']},
    'Save images and location in articles': {'nodes': ['Node7', 'Node8', 'Node4']},}
    
    node_list=['Save images and location in articles']

    storage_instance.graph = new_graph
    storage_instance.intent_tree = new_intent_tree
    storage_instance.mapping = new_mapping
    enhanced_mapping = complete_mapping_with_internal_links(new_mapping, new_graph)
    
    # Update the mapping with the enhanced version
    mapping = enhanced_mapping
    
    # Initialize the simplifier with our data
    simplifier = DataSimplifier(new_intent_tree, new_graph, new_mapping)
    
    # Perform the simplification based on the focus nodes
    simplifier.simplify(node_list)
    
    # Get and return the simplified graph
    simplified_graph = simplifier.get_simplified_graph()
    print(f"Simplified graph: {simplified_graph['nodes']}")
    simplified_graph['links'].append({'source': 'Node10', 'target': 'Node7', 'type': 'parse'})
    simplified_graph['links'].append({'source': 'Node9', 'target': 'Node4', 'type': 'extractText'})
    storage_instance.sim_graph = simplified_graph

    return {
                "status": "success", 
                "graph": storage_instance.graph,
                "sim_graph": simplified_graph,
                "intent_tree": new_intent_tree,
                "changed_nodes": node_list
            }
    try:
