        ...
        if (text && graphManager.oldGraph) {
            ...
            try {
                ...
                if (result.status === "success" && result.graph) {
                    // Use server returned changed nodes if available
                    const changedNodes = result.changed_nodes || [];
                    console.log("Changed nodes received from server:", changedNodes);
                    ...
                    // Render new_graph to top panel with highlighting modified nodes
                    const oldsvg = graphManager.renderGraph(
                        ...
                        new Set(changedNodes) // Pass modified node IDs as a Set for better performance
                    );
                    console.log("Old SVG:", oldsvg);
                    // oldsvg.append('text').attr('x', 200).attr('y', 120).text('Modified Graph').attr('font-size', '20px').attr('fill', 'black');
                    
                    // Render sim_graph to bottom panel
                    const newsvg = graphManager.renderGraph(
                    ...
                    // newsvg.append('text').attr('x', 200).attr('y', 120).text('Simulated Graph').attr('font-size', '20px').attr('fill', 'black');
                    // Add modified nodes to the graph
                    
                    // Update stored graphs
                    graphManager.currentGraph = formattedSimGraph;
                    graphManager.oldGraph = formattedNewGraph;
                    
                    // Render intent tree with changed nodes
                    if (result.intent_tree) {
                        graphManager.renderIntentTree(
                            result.intent_tree, 
                            changedNodes
                        );
                    }
    ...
    submitButton.addEventListener('click', async () => {
        ...
        if (text) {
            ...
            try {
                ...
                console.log("Old SVG:", oldsvg);
                // setTimeout(()=>{
                //     oldsvg.append('text').attr('x', 200).attr('y', 120).text('FUCK').attr('font-size', '20px').attr('fill', 'black');
                // }, 5000)
                ...
                // console.log('New svg 555', newsvg)
                // setTimeout(()=>{
                //     newsvg.append('text').attr('x', 200).attr('y', 120).text('FUCK').attr('font-size', '20px').attr('fill', 'black');
                //     const svgElement = newsvg.node()
                    
                //     const originalWidth = 500
                //     svgElement.setAttribute('width', parseInt(originalWidth) + 1);
                //     // Revert the attribute back to the original value
                ...
                // Add this code to render the intent tree
                if (result.intent_tree) {
                    graphManager.renderIntentTree(
                        result.intent_tree, 
