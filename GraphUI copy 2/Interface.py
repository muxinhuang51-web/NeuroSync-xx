...
class Storage:
    ...
    def diff_intent_tree(self):
        ...
        # 提取所有键
        keys_old = list(extract_keys(storage_instance.old_intent_tree))
...
def GraphCallFunction(old_graph, user_prompt):
    try:
        ...
        return {"status": "success", "graph": new_graph, "sim_graph": sim_graph}
...
def call_LLM(chat_history, new_graph):
    ...
    try:
        ...
        user_prompt += f"当前用户输入: {current_user_input}\n\n"
        user_prompt += f"任务图:\n{json.dumps(new_graph, indent=2, ensure_ascii=False)}\n\n"
        user_prompt += "你需要基于上述任务图中描述的任务执行逻辑和子任务完成和用户需求，生成完整的代码实现。请确保代码可以实际运行并满足用户需求。"
