class GraphManager {
    ...
    renderGraph(svg, graph, isEditable, isInitialRender = false, previewScale = 1, commonNodeIds = null, disableExtension = false, disableEditing = false, modifiedNodeIds = null) {
        ...
        // Ensure existing defs are removed to avoid duplicates
        ...
        const nodeHeight = 80 * previewScale;  // CHANGE: Same height regardless of preview
        ...
        const edgePaths = svg.selectAll('.edge')
            ...
            .append('path')
            .attr('class', 'edge')
            .attr('marker-end', `url(#${markerId})`) // Apply marker-end during initialization
            .style('stroke', '#999')
        ...
        const simulation = d3.forceSimulation(graph.nodes)
            ...
            .on("tick", ticked)
            .on("end", function() {
                // Ensure marker-end is applied after simulation ends
                edgePaths.attr('marker-end', `url(#${markerId})`);
            });
        ...
        if (isInitialRender) {
            ...
            // Manually call ticked to ensure nodes are rendered properly
            for (let i = 0; i < 20; i++) {
                ticked();
            }
        } else {
            // For updates, run more ticks to ensure proper positioning
            for (let i = 0; i < 20; i++) {
                simulation.tick();
                ticked(); // Explicitly call ticked after each simulation tick
            }
            simulation.stop();
        }
    
        // Add dispatch event so we can manually trigger ticks from outside
        svg.on("tick", ticked);
        ...
        if (isEditable && !disableExtension) {
            nodes.filter(d => !d.isExtended && (d.label && d.label.startsWith("Merged")))
                .append('circle')
            ...
            // For consistency, also update the extended nodes filter
            nodes.filter(d => d.isExtended && (d.label && d.label.startsWith("Merged")))
                .append('circle')
                .attr('class', 'extension-indicator')
                .attr('r', 8)
        ...
        function ticked() {
            ...
            .attr('d', function(d) {
                // 计算源和目标节点的位置
                const sourceX = typeof d.source === 'object' ? d.source.x : d.fx || 0;
                const sourceY = typeof d.source === 'object' ? d.source.y : d.fy || 0;
                const targetX = typeof d.target === 'object' ? d.target.x : d.fx || 0;
                const targetY = typeof d.target === 'object' ? d.target.y : d.fy || 0;
                
                // 计算边的路径
                return `M${sourceX},${sourceY}L${targetX},${targetY}`;
            })
            .attr('marker-end', `url(#${markerId})`); // 确保每次更新时都保持箭头属性
            ...
            // Update nodes with proper error checking - MAKE SURE THIS RUNS
        ...
        if (isEditable && !disableEditing) {
            ...
            addButton.append("text")
                .attr("text-anchor", "middle")
                .attr("dominant-baseline", "central") // Improve text centering
                .attr("fill", "white")
                .attr("font-size", "24px")
                .text("+");
            
            // Add extra visibility for debugging
            console.log("Add node button created at position:", width - 40, 40);
