...
def call_LLM(chat_history, new_graph):
    ...
    try:
        ...
        if storage_instance.baseline:
            ...
            user_prompt += "你需要基于上述任务图中描述的任务执行逻辑和子任务完成和用户需求，生成完整的代码实现。请确保代码可以实际运行并满足用户需求。同时标记code和给出graph中node的对应关。 Please response in English！"
...
def modify(user_input, new_graph):
    ...
    new_intent_tree= {'Fetch target webpage content via HTTP': {'Parse HTML with BeautifulSoup': {'Extract main text from web articles': {},
                                                                                'Find all images in web articles': {},
                                                                                'Save images location in articles': {}}}}
    ...
    'nodes': [{'description': 'Fetch target webpage content via HTTP',
            ...
            {'description': 'Extract main text from web articles',
                'id': 'Node3'},
            {'description': 'Save text with titles to Markdown', 'id': 'Node4'},
    ...
    new_mapping =    {'Extract main text from web articles': {'nodes': ['Node1', 'Node2', 'Node3']},
    ...
    'Save images location in articles': {'nodes': ['Node7']}}
    
    node_list=['Save images location in articles']
